...
case class IdentifierCondition() extends TokenCondition {
  def apply(position: Int, input: String): Option[(Int, TokenType)] =
    val eow = endOfWord(position, input)
    if(input.substring(position, eow+1).matches("[a-zA-Z_][a-zA-Z0-9_]*")) Some(eow, IDENTIFIER.apply()) else None
}

case class LiteralNumberCondition() extends TokenCondition {
  def apply(position: Int, input: String): Option[(Int, TokenType)] =
    val eow = endOfWord(position, input)
    if(input.substring(position, eow+1).matches("-?[0-9.,]+")) Some(eow, LITERALNUMBER.apply()) else None
}

case class LiteralStringCondition() extends TokenCondition {
  def apply(position: Int, input: String): Option[(Int, TokenType)] =
    val eow = endOfWord(position, input)
    if(input.substring(position, eow+1).matches("[\"-\'][^_]*[\"-\']")) Some(eow, LITERALSTRING.apply()) else None
